...
ifeq ($(strip $(COMPILER)),ADS)
  DIR_ARM        =  c:\arm\adsv1_2  #progra~1
  DIR_ARM       :=  $(strip $(DIR_ARM))
...
#Threedo
ifeq ($(strip $(THREEDO_APP_SUPPORT)), TRUE)
  COM_DEFS    += __THREEDO_APP_SUPPORT__
  COMMINCDIRS += plutommi\MtkApp\Threedo\ThreedoInc
  COMPLIST    += Threedo
  CUS_REL_SRC_COMP += Threedo
endif

#MQTT
ifeq ($(strip $(MQTT_APP_SUPPORT)), TRUE)
  COM_DEFS    += __MQTT_APP_SUPPORT__
  COMMINCDIRS += plutommi\MtkApp\Mqtt\MQTTPacket\src
  COMMINCDIRS += plutommi\MtkApp\Mqtt\MQTTClient-C\src
  COMMINCDIRS += plutommi\MtkApp\MtkApp\MQTTClient-C\mtk
  COMPLIST    += Mqtt
  CUS_REL_SRC_COMP += Mqtt
endif
...
ifdef XD_NO_POWER_KEY
  ifeq ($(strip $(XD_NO_POWER_KEY)),TRUE)
    COM_DEFS    += __XD_NO_POWER_KEY__
  endif
endif
